...
export function Bracket({ matches, currentUserId }: Props) {
    ...
    <div className="overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
      ...
      >
        {rounds.map((r) => {
          ...
          return (
            <div key={r} className="flex flex-col gap-3 min-w-[220px]" role="listitem">
              <h3 className="text-xs uppercase tracking-wider text-muted px-1">
                {isFinal
                  ? "Final"
                  : r === maxRound - 1 && roundMatches.length === 2
                    ? "Semifinals"
                    : `Round ${r}`}
              </h3>
...
function MatchCard({
  ...
  const card = (
    <div
      className={cn(
        "rounded-xl border bg-surface/70 p-2.5 text-sm w-full transition",
        involvesUser
  ...
  if (!match.playerB || match.status === "completed") {
    // No interaction needed for byes / done matches (still allow opening).
    return (
...
function PlayerRow({
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-between px-1 py-1 rounded",
